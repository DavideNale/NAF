...
def sortIR3(pos, ir, numXYZ, posX=None, posY=None, posZ=None):
    ...
    for i in range(numXYZ[2]):
        for j in range(numXYZ[1]):
            xIdx = np.where(
                np.isclose(pos[:, 1], posY[j]) & np.isclose(pos[:, 2], posZ[i])
            )[0]
            sorter = np.argsort(pos[xIdx, 0])
            xIdxSort = xIdx[sorter]
...
def movWave(sessionPath, x, y, ir, samplerate, start=None, end=None, downSampling=None):
    ...
    def animate(i):
        ...
        ax.set_title(
            "Sample: " + str(i) + ", Time: " + "{:.3f}".format(currentTime), fontsize=14
        )
        return (cax,)

    anim = animation.FuncAnimation(
        fig, animate, interval=15, frames=end - start - 1, blit=True
    )

    anim.save(
        sessionPath.joinpath("wave_mov.mp4"), writer="ffmpeg", fps=15, bitrate=1800
    )
